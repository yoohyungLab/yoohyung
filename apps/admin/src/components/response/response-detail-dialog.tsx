			...
			<DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
				...
				<div className="space-y-6">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div className="space-y-3">
							...
							<div className="space-y-2 text-sm">
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">응답 ID:</span>
									<span className="font-mono text-xs">{response.id}</span>
								</div>
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">테스트:</span>
									<span className="text-right max-w-xs truncate">{response.test_title}</span>
								</div>
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">카테고리:</span>
								...
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">결과 유형:</span>
									<Badge variant="default" className="whitespace-nowrap">
										{response.result_name || '없음'}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">점수:</span>
						...
						<div className="space-y-3">
							<h4 className="font-semibold text-lg border-b border-neutral-200 pb-2 text-neutral-900">환경 정보</h4>
							<div className="space-y-2 text-sm">
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">디바이스:</span>
									<span>{ResponseUtils.formatDeviceType(response.device_type)}</span>
								</div>
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">IP 주소:</span>
									<span className="font-mono text-xs">{String(response.ip_address || '알 수 없음')}</span>
								</div>
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">User Agent:</span>
									<span className="text-xs max-w-xs truncate">
										{response.user_agent ? response.user_agent.slice(0, 30) + '...' : '알 수 없음'}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="font-medium text-neutral-600">리퍼러:</span>
