...
import type { TestQuestion, TestResult } from '@repo/supabase';
...
export const PreviewStep: React.FC<PreviewStepProps> = ({ testData, questions, results, selectedType }) => {
    ...
    const checklistItems = [
        ...
        { check: results.every((r: any) => r.description?.length > 20), text: '모든 결과에 충분한 내용이 있나요?' },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                <CardHeader>
                    <CardTitle className="text-2xl text-blue-900">{testData.title || '제목 없음'}</CardTitle>
                    <p className="text-blue-700 mt-2">{testData.description || '설명 없음'}</p>
                                ...
                                <div key={index} className="p-3 bg-gray-50 rounded-lg">
                                    <div className="flex items-start gap-3">
                                        <div
                                            ...
                                            style={{ backgroundColor: result.theme_color || '#3B82F6' }}
                                        />
                                        <div className="flex-1">
            ...
            {/* 체크리스트 */}
            <Card className={`border-2 ${allChecksPassed ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
                <CardHeader>
                    <CardTitle className={`flex items-center gap-2 ${allChecksPassed ? 'text-green-800' : 'text-amber-800'}`}>
