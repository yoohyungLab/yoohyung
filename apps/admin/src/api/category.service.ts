...
export const categoryService = {
    ...
    async createCategory(category: { name: string; description?: string; sort_order?: number; slug: string; is_active?: boolean }) {
        const categoryData = {
            ...category,
    ...
    async updateCategoryStatus(id: number, is_active: boolean): Promise<Category> {
        return this.updateCategory(id, { is_active });
    },

    async bulkUpdateStatus(categoryIds: number[], is_active: boolean): Promise<number> {
        const { data, error } = await supabase
            .from('categories')
