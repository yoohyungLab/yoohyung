...
>
> - 📋 **Core Guidelines**: This document (~1,800 tokens) - Always loaded
> - 📚 **Code Examples**: `CLAUDE-EXAMPLES.md` - Reference when implementing complex patterns
> - ⚠️ **Anti-patterns**: `CLAUDE-ANTIPATTERNS.md` - Reference when reviewing or debugging code
>
> Only reference additional documents when specifically needed for the task.
...
# Packages List When Implementing API Integration
...
- `path-to-regexp`: For route path matching.
...
- Not used: `apps/admin`, `apps/server`, `apps/storybook`

- Import shared modules from `packages`

- API reference: `packages/data/user-api` (do not use `packages/data/admin-api`)

  - Auto-generated by OpenAPI Generator

  - `packages/data/user-api/api/api.ts`: API export index

  - `packages/data/user-api/api/apis/*`: generated API files

  - `packages/data/user-api/api/models/index.ts`: model export index
...
- Route files location: `apps/react/src/app/`

- File-based routing: `index.tsx` → `/`, `about.tsx` → `/about`

- Type safety: TypeScript integration

- Auto-generation: `routeTree.gen.ts` is auto-updated on build (do not edit manually)
...
│   │   ├── state/*                 # Local state hooks (useState-based)
│   │   ├── ui/*                    # UI-related hooks
│   │   ├── form/*                  # React Hook Form
│   │   └── utils/*                 # Utilities
│   ├── constants/*                  # Constants
│   └── store/*                      # Zustand stores (global state)
└── shared/                          # Cross-feature code (do not modify without explicit request)
```
...
- **Use generated API and models from packages/data/user-api**

- **API client is configured in @/shared/libs/api-client**
...
```typescript
// features/auth/hooks/api/useUserQuery.ts
import { useQuery } from '@tanstack/react-query';
import { UsersApi, type GetUserResDto } from '@data/user-api/api';
import { apiClient } from '@/shared/libs/api-client';

const usersApi = new UsersApi(apiClient.configuration, apiClient.basePath, apiClient.axios);

export const useUserQuery = (userId: string) => {
	return useQuery<GetUserResDto>({
		queryKey: ['user', userId],
		queryFn: async () => {
			const response = await usersApi.usersGetUser({ userId });
...
```typescript
// features/auth/hooks/api/useLoginMutation.ts
import { useMutation } from '@tanstack/react-query';
import { AuthApi, type PostAuthReqDto, type PostAuthResDto } from '@data/user-api/api';
import { apiClient } from '@/shared/libs/api-client';

const authApi = new AuthApi(apiClient.configuration, apiClient.basePath, apiClient.axios);

export const useLoginMutation = () => {
	return useMutation<PostAuthResDto, Error, PostAuthReqDto>({
		mutationFn: async (data) => {
			const response = await authApi.authPostAuth({ postAuthReqDto: data });
			return response.data;
		},
		onSuccess: (response) => {
...
- ✅ ALWAYS use generated types from `@data/user-api/api`

- ✅ ALWAYS import Response types (e.g., `PostAuthResDto`)

- ✅ ALWAYS specify generics in useMutation/useQuery

- ✅ ALWAYS return `response.data` from API calls

- ✅ Create API instances at module level (not inside hooks)

- ❌ NEVER create custom interfaces for API parameters
...
import { type PostAuthReqDto } from '@data/user-api/api';
```

### ❌ Missing Response Types

```typescript
// WRONG - No response type
return useMutation({
  mutationFn: async (data) => { ... }
})

// CORRECT - Specify all generics
return useMutation<PostAuthResDto, Error, PostAuthReqDto>({
  mutationFn: async (data) => { ... }
})
```

### ✅ Handling Partial Parameters

When user input doesn't match full API requirements:

```typescript
// Option 1: Accept partial input, add defaults in mutationFn
export const usePhoneVerificationMutation = () => {
	return useMutation<PostVerificationResDto, Error, string>({
		mutationFn: async (phone) => {
			const data: PostVerificationPhoneReqDto = {
				phone,
				forAndroid: false, // Default value
				type: PostVerificationPhoneReqDtoTypeEnum.Register, // Default value
			};
			const response = await api.verificationsPhone({ postVerificationPhoneReqDto: data });
			return response.data;
		},
	});
};

// Option 2: Use Pick type for partial input
type PhoneInput = Pick<PostVerificationPhoneReqDto, 'phone'>;

export const usePhoneVerificationMutation = () => {
	return useMutation<PostVerificationResDto, Error, PhoneInput>({
		mutationFn: async ({ phone }) => {
			// ... same as above
		},
	});
