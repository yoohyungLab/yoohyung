...
export function TestQuestionScreen({
	...
	return (
		<main className={`min-h-screen flex items-center justify-center px-4 py-8 bg-gradient-to-br ${theme.gradient}`}>
				...
				<header className="relative mb-6">
					<div className="flex items-center justify-between mb-3">
						...
						{progress.currentQuestionIndex > 0 && (
							<button
								onClick={onPrevious}
								className={`px-3 py-1.5 bg-${theme.primary}-100 text-${theme.primary}-700 rounded-full text-xs font-bold hover:bg-${theme.primary}-200 active:scale-95 transition-all`}
							>
								← 이전
							</button>
						)}
					...
					<div className="h-3 bg-gray-100 rounded-full overflow-hidden">
						<div
							...
							style={{ width: `${(progress.currentQuestionIndex / progress.totalQuestions) * 100}%` }}
						/>
					</div>
				</header>
