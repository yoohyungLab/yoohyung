...
// 시작 횟수는 "시작하기" 버튼 클릭 시 증가 (test-start-screen에서 처리)
// 완료 횟수는 결과 저장 시 증가 (saveResult에서 처리)
...
export function useTestResultVM() {
				...
				const matchByScore = (results: TestResult[]) => {
					for (const result of results) {
						if (result.match_conditions) {
							const conditions = result.match_conditions as MatchConditions;
	...
	const saveTestResult = useCallback(
		async (resultData: TestResultData) => {
			try {
				setIsLoading(true);
				setError(null);
