...
import { useShareToast } from '@/shared/hooks';
import { isMobileDevice } from '@/shared/lib/test-utils';
...
export function TestCTAButtons(props: TestCTAButtonsProps) {
	...
	const handleShare = useCallback(async () => {
		...
		const shareUrl = isBalanceGame
			? `${window.location.origin}/tests/${testId}?ref=share`
			: `${window.location.origin}/tests/${testId}/result?ref=share`;
			...
			<div className={`space-y-4 pb-8 ${className}`}>
				...
				<div className="grid grid-cols-2 gap-3">
					...
					<Button
						onClick={handleShare}
						size="xl"
						className="bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 bg-[length:200%_100%] hover:bg-right text-white font-black text-[15px] py-4 px-5 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500"
					>
